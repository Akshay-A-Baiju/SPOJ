...
// URL: https://www.spoj.com/problems/SERGRID/
...
void solve()
{
	...
	while (!q.empty())
	{
		...
		ll y=q.front().ss;
		q.pop();
		if (x==n-1 && y==m-1) { cout<<min_moves[x][y]; return; }
		ll k=grid[x][y]-'0',newx,newy;
		// 4 directions
		newx=x-k;
		if (newx>=0 && min_moves[newx][y]==-1)
		{
			q.push({newx,y});
			min_moves[newx][y]=min_moves[x][y]+1;
		}
		newx=x+k;
		if (newx<n && min_moves[newx][y]==-1)
		{
			q.push({newx,y});
			min_moves[newx][y]=min_moves[x][y]+1;
		}
		newy=y-k;
		if (newy>=0 && min_moves[x][newy]==-1)
		{
			q.push({x,newy});
			min_moves[x][newy]=min_moves[x][y]+1;
		}
		newy=y+k;
		if (newy<m && min_moves[x][newy]==-1)
		{
			q.push({x,newy});
			min_moves[x][newy]=min_moves[x][y]+1;
		}		
