...
// URL: https://www.spoj.com/problems/BITPLAY/
// Memory Limit: 1536 MB
// Time Limit: 1000 ms
// Problem tag: bits, easy
// TC: O(t)
// SC: O(1)
...
void solve()
{
	...
	ll ans=n-1;	// maximum odd no less than n
	ll stbits=__builtin_popcountll(ans);
	if (stbits<=k) { cout<<ans; return; }
	// else remove the set bits from 1st bit to previous
	ll x=2;
	while (stbits>k)
	{
		if (ans&x)	// if set
