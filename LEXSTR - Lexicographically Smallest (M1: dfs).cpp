...
// Problem: Lexicographically Smallest
// Contest: SPOJ - Classical
// URL: https://www.spoj.com/problems/LEXSTR/
...
void dfs(ll node, vbool &vis, vll adj[], vll &comp)
{
	vis[node]=1;
	comp.pb(node);
...
void solve()
{
	...
	for (ll i=0;i<n;i++)
	{
		if (!vis[i])
		{
			vll comp; // indices in the connected component
			dfs(i,vis,adj,comp);
			// sort the charcters in order
			vll chars;
			for (auto &it: comp) chars.pb(s[it]);
			sort(all(chars));
			sort(all(comp));
			for (ll i=0;i<comp.size();i++) s[comp[i]]=chars[i];
		}
	}
	cout<<s;
}
