...
// URL: https://www.spoj.com/problems/KNMOVE/
// Memory Limit: 1536 MB
// Time Limit: 1000 ms
// Problem tag: bfs, precomputation
// TC:
// SC:
...
vvll min_moves(1000, vll (1000,-1));

void preCompute()
{
  // bfs on grid starting from (0,0) to find minimum moves to a particular cell from (0,0)
	queue <pll> q;
	q.push({0LL,0LL});
	min_moves[0LL][0LL]=0;
	while (!q.empty())
	{
		...
		q.pop();
		// 8 moves by a knight
		vpll kmoves={{-2,1},{-1,2},{1,2},{2,1},{2,-1},{1,-2},{-1,-2},{-2,-1}};
		for (auto &pr: kmoves)
		{
			ll newx=x+pr.ff;
			ll newy=y+pr.ss;
			if (newx<0 || newx>=1000 || newy<0 || newy>=1000) continue;
			if (min_moves[newx][newy]==-1)
			{
				q.push({newx,newy});
				min_moves[newx][newy]=min_moves[x][y]+1;
			}			
		}		
	}
}
